...
export default function ShopProfile() {
  ...
  const { currentShopId, viewProduct, toggleWish, isWished } = useStore();
  ...
  if (error || !s) return (
    <div className="text-center py-20">
      ...
      <button onClick={() => router.push('/explorer')} className="text-[#8d5524] font-bold hover:underline">
        ← Back to Explore
      </button>
    </div>
  );
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {s.products.map((p, i) => (
                <div key={p.id}
                  ...
                  onClick={() => {
                    viewProduct(s.id, p.id);
                    router.push(`/explorer/product/${s.id}/${p.id}`);
                  }}>
